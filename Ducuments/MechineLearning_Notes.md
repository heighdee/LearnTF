...
1. **数据集（Data set）：样本集合**
   ...
   1. 样本/示例（sample / instance）：关于一个对象的描述
   2. 属性/特征（attribute/feature）：反应被描述对象某方面的表现或者性质的事项。具体对象具体特征的取值成为 *属性值/特征值*

   一个 *样本* 对应一个*属性空间/样本空间/输入空间* 中的某个点，每个点对应一个坐标向量，所以一个示例也可以称为一个*特征向量*

   3. 标记（label）：训练样本的结果信息。拥有标记信息的 *示例* 称为 *样例*（example）
...
5. **归纳偏好：机器学习算法在学习过程中对某种烈性假设的偏好**
   ...
   归纳偏好的实质：是学习算法本身所做出的关于“什么样的模型更好”的假设。

   算法偏好的一般性原则：奥卡姆剃刀（如无必要，勿增实体），即“若有多个假设与观察一致，选择最简单的那个”

   免费的午餐定理：当所有问题出现的机会相同时，所有的学习算法的期望性能都跟随机猜想差不多。但实际情形并非如此，我们只为某个具体问题寻找解决方案。要谈论算法的优劣，必须针对具体的学习问题。
...
我们希望获得**泛化误差小**的学习器，实际能做的是努力使**经验误差最小化**。当学习器把训练数据学的太好的时候，很可能产生“过拟合”现象， 过拟合的学习器泛化能力差。一般各类学习算法都会有缓解过拟合风险的措施。
...
1. 在测试集上的预测性能并不等价于学习器的泛化性能
2. 测试集的选择和测试集的优劣非常影响结果
...
偏差方差分解：用来解释学习算法的泛化性能。

学习器的泛化误差可以分解为偏差、方差和噪声之和。
...
1. P66 为什么 对数几率回归 不需要事先假设数据分布
2. P70 当两类数据同先验、满足高斯分布且协方差相等时，LDA 可达最优解。
3. 
